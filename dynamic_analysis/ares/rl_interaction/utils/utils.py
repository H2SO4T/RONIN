...
class EmulatorLauncher:
    ...
    def start_emulator(self):
        ...
        if self.emu == 'normal':
            if self.speedup:
                self.process = subprocess.Popen([f'{os.environ["ANDROID_HOME"]}/emulator/emulator',
                                                 f'{self.device_name}',
                                                 '-port', f'{self.android_port}', '-gpu', 'on', '-no-audio'])
                time.sleep(15)
            else:
                self.process = subprocess.Popen([f'{os.environ["ANDROID_HOME"]}/emulator/emulator',
                                                 f'{self.device_name}',
                                                 '-port', f'{self.android_port}', '-no-audio', '-no-snapshot',
                                                 '-no-boot-anim', '-wipe-data', '-gpu', 'on'])
                time.sleep(50.0)
        else:
