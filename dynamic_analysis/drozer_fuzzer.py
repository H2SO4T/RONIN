...
#see LICENSE and LICENSE.drozer for details


from drozer.modules import Module, common
from drozer.modules import Module, app
...
def log_in_logcat(log, device):
    '''
    Function which make an entry in the Android system
    logcat with the running intent
    '''
    if device is not "":
        print device
        getoutput("adb -s %s logcat -c " % (str(device)))
        log_command = "adb -s %s shell  log -p f -t %s" % (str(device),
                      str(log))
...
def save_logcat(fuzz_type, package, component, device, current_dir):
    '''
    Function for saving the system logcat
    after every intent generated
    '''
    result_folder = str(current_dir) + "/Results_" + str(fuzz_type) + \
        "_" + str(package)
    ...
    parse_logcat(file_name, str(fuzz_type), str(component), str(package),
                 current_dir)
...
def random_value(extra_type):
    '''
    Function for generating random value for
    extra_value parameter according to the extra_type
    '''
...
class Fuzzinozer(Module, common.PackageManager, common.Filters,
                 common.IntentFilter):
    '''
    ...
    def add_arguments(self, parser):
        parser.add_argument("--package_name", help="specify name of" +
                            "package to test ")
        parser.add_argument("--test_all", action='store_true',
                            help="test all packages")
        parser.add_argument("--broadcast_intent", action='store_true',
                            help="send broadcast intent; you also need" +
                            "to select package_name or test_all params")
        parser.add_argument("--fuzzing_intent", action='store_true',
                            help="send intent with random action, category," +
                            "flag; you also need to select --package_name" +
                            "or --test_all params")
        parser.add_argument("--complete_test", action='store_true',
                            help="test with all actions, categories, flags," +
                            "extras existent in Android; you also need to" +
                            " select --package_name or --test_all params")
        parser.add_argument("--select_fuzz_parameters",
                            help="give the parameters you want to fuzz " +
                            "(action, category, flag, extras);you need " +
                            "to select --fuzzing_intent and--package_name or" +
                            " --test_all params")
        parser.add_argument("--save_state", action='store_true',
                            help="save the running state from" +
                            "complete_test session")
        parser.add_argument("--reload", help="reload the running state" +
                            " parameters in terms of component, action," +
                            " category, flag, key as indices")
        parser.add_argument("--run_seed",
                            help="select the seed file you want to run")
        parser.add_argument("--device", help="used only for automated tests")
        parser.add_argument("--template_fuzz_parameters_number",
                            help="give the number of the parameters you " +
                            "want to fuzz; you also need to select " +
                            "--fuzzing_intent and --package_name or" +
                            " --test_all params")
        parser.add_argument("--dos_attack", help="give the number of" +
                            " intents you want to send")
    ...
    def dos(self, intents):
        ...
        fileName = "buffer.sh"
    #hardcoded package activity. Bluetooth activity does not bring any UI in foreground
        pack_act = "com.android.bluetooth/.opp.BluetoothOppLauncherActivity"

        for i in range(intents):
            with open(fileName, "w + ") as f:
                rand_int_f = random.randint(-2147483648, 2147483647)
                ...
                f.write("am start -n " + pack_act + " -f " + str(rand_int_f) +
                        " -a " + string_generator(rand_size_a) + " -c " +
                        string_generator(rand_size_c) + " -d " +
                        string_generator(rand_size_d) + "-e " +
                        string_generator(rand_size_ek) + " " +
                        string_generator(rand_size_ev))
    ...
    def test_all_possible(self, package_name, device, current_dir,
                          save_state, reload_option, aux_num):
        '''
                        ...
                        for fl in flags[aux_fl:]:
                            for key in extra_keys[aux_key:]:
                                for extra_type in extra_types:
                                    ...
                                    uri = generate_random_uri()
                                    if (save_state):
                                        with open(str(current_dir +
                                                  "/complete_test-saved.txt"),
                                                  'w + ') as f:
                                            f.write(str(activities.index(component)) +
                                                    " " + str(actions.index(ac)) +
                                                    " " + str(categories.index(cat)) +
                                                    " " + str(flags.index(fl)) + " " +
                                                    str(extra_keys.index(key)))
                                            f.close()
                                    self.run_intent(package_name, component,
                                                    uri, ac, cat, fl,
                                                    extra_type, key,
                                                    extra_value, device)
